...
typedef struct aNode{
	T_elt words[WORDS_MAX]; // liste de mots contenu dans le noeud 
    int i_words;  // indice de la liste de mots
...
int nbAnagrams(T_avl root){
	// nb de anagrammes d'un arbre (y compris les feuilles) 
	if(root == NULL) return 0;

	int nbWordsNode = getSizeRec(root-> words); 
	
	int test = nbWordsNode > 1 ? nbWordsNode : 0;

	return test + nbAnagrams(root->l) + nbAnagrams(root->r);
